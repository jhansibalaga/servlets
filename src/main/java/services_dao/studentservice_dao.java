...
public class studentservice_dao {
	...
	//here list is using because the database contains heterogeneous data
	public List<student> fetchall(){
		...
		try {
			...
			PreparedStatement pmst = conn.prepareStatement(sql);
			 ...
			 while (rs.next()) {  //  we don't know the no of iterations so we use while loop
				 ...
				 s = new student();
				 s.setId(rs.getInt(1)); //while fetching first we have to set the data means the database know what we have to fetch and then we get the required data
				 s.setName(rs.getString(2));
				 s.setEmail(rs.getString(3));
				 s.setBranch(rs.getString(4));
				 s.setPhoneno(rs.getLong(5));
				 
				 li.add(s); // we are adding all fetched data to list
			}
		...
		return li; // so, returning list contains all fetched data
	}
	...
	public student fetchbyid(int id) {
		...
		try {
			...
			PreparedStatement pmst = conn.prepareStatement(sql);
			pmst.setInt(1, id); //first we have to insert the id to get
			  ResultSet rs = pmst.executeQuery();
			  while (rs.next()) {
				s = new student();
				s.setId(rs.getInt(1));
	...
	public boolean update(student st) {
		...
		try {
			...
			pmst.setString(1,st.getName()); // while updating first we have 
